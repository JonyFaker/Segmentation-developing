...
def main(args):
    ...
    ori_net_encoder = builder.build_encoder(
    ...
    ori_net_decoder = builder.build_decoder(
        arch=args.arch_decoder,
        fc_dim=args.fc_dim,
        num_class = 150,
        weights=args.weights_decoder)
    # ori_net_decoder = ori_net_decoder.load_state_dict(torch.load(args.weights_decoder)['state_dict'])
    ori_dict = ori_net_decoder.state_dict()
    net_decoder = builder.build_decoder(
        arch=args.arch_decoder,
        fc_dim=args.fc_dim,
        num_class=args.num_class,
        weights='')
    new_dict = net_decoder.state_dict()
    for k, v in ori_dict.items():
        print ('k:{} conrespond to: {}'.format(k, v.size()))
    ori_dict = {k: v for k, v in ori_dict.items() if k in new_dict and k not in ['conv_last.4.weight', 'conv_last.4.bias', 'conv_last_deepsup.weight', 'conv_last_deepsup.bias'] }
    new_dict.update(ori_dict)
    net_decoder.load_state_dict(new_dict)
