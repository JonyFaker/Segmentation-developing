...
class TrainDataset(torchdata.Dataset):
    def __init__(self, odgt, opt, max_sample=-1, batch_per_gpu=1):
        ...
        self.img_transform = transforms.Compose([
            ...
            transforms.Normalize(mean=[110.32773 , 116.44132 , 125.804115], std=[67.129944, 66.37249 , 65.02814])
            ])
    ...
    def __getitem__(self, index):
        ...
        # print("this_short_size:{ }".format(this_short_size))
        ...
        batch_resized_height = 480
        batch_resized_width = 640
        ...
        batch_edges = torch.zeros(self.batch_per_gpu, batch_resized_height // self.segm_downsampling_rate, \
        ...
        for i in range(self.batch_per_gpu):
            ...
            img = imresize(img, (480, 640), interp='bilinear')
            segm = imresize(segm, (480, 640), interp='nearest')
            edge = cv2.Canny(img, 50, 150)    # boundary info
            ...
            # to avoid edge misalignment, added by me for edge info
            edge_rounded_height = round2nearest_multiple(edge.shape[0], self.segm_downsampling_rate)
            edge_rounded_width = round2nearest_multiple(edge.shape[1], self.segm_downsampling_rate)
            edge_rounded = np.zeros((edge_rounded_height, edge_rounded_width), dtype='uint8')
            edge_rounded[:edge.shape[0], :edge.shape[1]] = edge

            edge = imresize(edge_rounded, (edge_rounded.shape[0] // self.segm_downsampling_rate, \
                                           edge_rounded.shape[1] // self.segm_downsampling_rate), \
                            interp='nearest')
            ...
            batch_edges[i][:edge.shape[0], :edge.shape[1]] = torch.from_numpy(edge.astype(np.int)).long()

        batch_segms = batch_segms - 1 # label from -1 to 149
        output = dict()
        output['img_data'] = batch_images
        output['seg_label'] = batch_segms
        output['edge_data'] = batch_edges

        # print("batch_images:{}, batch_segms:{}, batch_edges:{}".format(batch_images.size(), batch_segms.size(), batch_edges.size()))
...
class ValDataset(torchdata.Dataset):
    ...
    def __getitem__(self, index):
        ...
        segm = imread(segm_path)
        segm = imresize(segm, (480, 640), interp='nearest')  ## added by jf
